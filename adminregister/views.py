...
def _normalize_iso(dt):
    ...
    if isinstance(dt, datetime):
        ...
        if is_naive(d):
            ...
            # dicts (report/fix_vulnerability docs), written with plain
            # datetime.utcnow() — always genuinely UTC, never in
            # settings.TIME_ZONE. make_aware() with no explicit timezone
            # assumes the CURRENT TIME_ZONE (Asia/Kolkata) as the value's
            # origin, which mislabels an already-UTC value as if it were
            # IST — shifting it by 5:30 in the wrong direction. Pass utc
            # explicitly so it's labeled correctly and converts to real
            # IST wherever it's displayed.
            d = make_aware(d, utc)
        return d.isoformat()
    return str(dt)
...
class SupportRequestByReportAPIView(APIView):
    ...
    def post(self, request, report_id):
        ...
        with MongoContext() as db:
            ...
            # Real request: once a support request is closed, no new
            # message should be addable to it — the GET side already
            # computes this SAME "effective closed" status (either the
            # request's own status field, or its linked vulnerability
            # having been closed separately) to show "Closed" in the
            # detail view; this POST had no matching check at all, so a
            # reply could still be sent right through a ticket the admin
            # was shown as already closed.

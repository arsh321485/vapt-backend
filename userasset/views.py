...
def _get_team_plugin_names(db, report_id, teams_lower):
    ...
    upload almost every team-scoped view built on this helper (asset
    lists, vulnerability lists, register) showed wrong/incomplete data
    until AI generation fully caught up. Falls back to
    team_utils.infer_assigned_team() (deterministic keyword match, no AI
    call) whenever a plugin has no card, or a card with no/invalid team —
    same fallback already proven correct elsewhere in the app.
    """
    ...
    def post(self, request, report_id, plugin_name):
        ...
        try:
            ...
            with MongoContext() as db:
                ...
                # Real bug report: the whole-asset hold/unhold/delete views
                # already bust the owning admin's dashboard cache after a
                # mutation, but this vulnerability-level hold never did —
                # Summary kept serving its last cached response (up to
                # 5 min) after a vuln hold, even though every other,
    ...
    def get(self, request, report_id):
        try:
            ...
            with MongoContext() as db:
                ...
                # so its asset_type resolves the same way UserReportAssetsAPIView does.
                asset_type_map = get_asset_type_map_for_report(
                    db, report_id,
                    [
                        {"host_name": (h.get("host_name") or "").strip(),

...
print("=" * 60)
print("Password Autocomplete Verify — Plugin 42057 — Windows")
...
def check_autocomplete(html: str, url: str) -> bool:
    """Check if all password fields have autocomplete=off."""
    page_ok = True
    ...
    for inp in password_inputs:
        if re.search(r'autocomplete=["\']off["\']', inp,
                     re.IGNORECASE):
    ...
    # Also check form tags
    form_tags = re.findall(r'<form[^>]*>', html, re.IGNORECASE)
    for form in form_tags:
        if re.search(r'autocomplete=["\']off["\']', form,
                     re.IGNORECASE):
            log.info("   ✅ Form has autocomplete=off: %s",
                     form[:80].strip())
        else:
            log.warning(
                "   ⚠️ Form missing autocomplete=off: %s",
                form[:80].strip()
            )
...
for url in URLS_TO_CHECK:
    ...
    try:
        ...
        try:
            ...
            log.warning("   ⚠️ Could not read response body")
...
if all_good:
    ...
    print("   Manually add autocomplete='off' to password fields in HTML")
print("=" * 60)
...
print("=" * 60)
print("SSL Self-Signed Certificate Verify — Windows")
print("Plugin 57582")
print("=" * 60)
...
try:
    ...
    if subject == issuer:
        print("❌ Certificate is SELF-SIGNED — not trusted!")
...
try:
    ...
    if result.stdout.strip():
        print("✅ Certificates in Windows Store:")
        print(f"   {result.stdout.strip()[:300]}")
    else:
        print("⚠️ No certificates in LocalMachine\\My store")
except Exception as e:
    print(f"❌ Could not check certificate store: {e}")

# ─── Check 5: Live SSL with verification ─────────────────────────
...
try:
    ...
    print("❌ Certificate is NOT trusted — still self-signed!")
    print("   → Replace with CA-signed certificate")
    all_good = False
except Exception as e:
...
if all_good:
    ...
    print("❌ VERDICT: FAIL — Certificate is still self-signed.")

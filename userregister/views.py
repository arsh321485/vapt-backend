    ...
    def get(self, request, fix_vuln_id):
        try:
            with MongoContext() as db:
                ...
                # description entirely, even though UserFixVulnerabilityCreateAPIView
                # already stores it on fix_doc at creation time — same
                # fallback chain used by the card/detail endpoints
                # (description -> description_points -> vuln_card's synopsis).
                description = (
...
class UserSupportRequestsByReportAPIView(APIView):
    ...
    Returns support requests for a given report that belong to
    the logged-in user's assigned teams (case-insensitive), scoped to the
    latest uploaded report of the admin who added this user.
    """
    permission_classes = [IsAuthenticated]
    ...
    def get(self, request, report_id):
        ...
        with MongoContext() as db:
            ...
            if not latest_doc:
                return Response(
                    {"detail": "No reports found for your admin's account"},
                    status=status.HTTP_404_NOT_FOUND,
                )
            if latest_doc.get("report_id") != str(report_id):
                return Response(
...
class UserSupportRequestsByHostAPIView(APIView):
    ...
    Returns support requests for a given asset (host_name) that belong to
    the logged-in user's assigned teams (case-insensitive), scoped to the
    latest uploaded report of the admin who added this user.
    """
    permission_classes = [IsAuthenticated]
